...
## Example: DII Backend (Live-Only)

DII (Data Infrastructure Insights) is the first non-ONTAP backend added to the
DataSource framework.  It demonstrates a minimal backend implementation that
validates the multi-API architecture without requiring cache infrastructure.

**ADR:** [ADR-0015](../decisions/0015-dii-backend-live-only-bare-array-envelope-offsetlimit-pagination.md)

### Key design choices

| Concern | DII approach | ONTAP comparison |
| :--- | :--- | :--- |
| Cache | None -- live-only | SQLite snapshot via `ClusterMetadataDB` |
| Response envelope | Bare JSON array (`records_path=""`) | `{"records": [...]}` dict |
| Pagination | `offset` + `limit` query params | `_links.next.href` HAL links |
| Authentication | API key header (`X-CloudInsights-ApiKey`) | HTTP Basic Auth per-cluster |
| `where_expressions` | `NotImplementedError` | SQL-like cache filters |
| Partial-fetch merge | Not supported (no cache fields) | Approach C: cache + live merge |

### Backend implementation

`DiiBackend` (`data/dii_backend.py`) is ~130 lines.  The `query()` method:

1. Rejects cache-only decisions and `where_expressions`.
2. Calls `DIIAPIClient.call_endpoint()` with filter dict as query params.
3. Iterates the bare array response, parsing each record via
   `parse_api_record()`.
4. Stamps `_fetched_fields` on every instance.

```python
class DiiBackend(Backend):
    def query(self, model_class, mapping, decision, cluster, filters, *,
              where_expressions=()):
        if decision.cache_fields and not decision.live_fields:
            raise NotImplementedError("DII backend does not support cache-only queries")
        if where_expressions:
            raise NotImplementedError("DII backend does not support where_expressions")
        # ... fetch and parse bare array ...
```

### Codegen surface

The full DII surface (191 endpoints) was generated by the OpenAPI codegen
pipeline (ADR-0008):

- **Models:** `src/pynetappfoundry/models/dii/` -- 191 `model.py` files
- **Mappings:** `src/pynetappfoundry/cache/dii/` -- 191 `mapping.py` files

All DII `TypeMapping` instances set `api_type="dii"` and `records_path=""`.

### What DII does NOT need

Because DII is live-only, the following ONTAP patterns are absent:

- No cache database tables or migrations
- No derived fields or post-fetch hooks
- No SSH/CLI fallback
- No `fields=*` query parameter support
- No partial-fetch merge algorithm
- No `where_expressions` support

This makes DII a useful reference for the simplest possible backend.
...
- [ADR-0015: DII Backend](../decisions/0015-dii-backend-live-only-bare-array-envelope-offsetlimit-pagination.md) (first non-ONTAP backend)
...
- `src/pynetappfoundry/data/dii_backend.py` -- DiiBackend (live-only, bare-array)
