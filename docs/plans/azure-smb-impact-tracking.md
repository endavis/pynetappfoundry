...
|---------|-----------------|--------------|-------------------|
| ZUSCULTAXCST010 | 14.4 seconds | 5 LIFs moved | 2 aggregates |
| ZUSCULTAXCST011 | 14.7 seconds | 6 LIFs moved | 4 aggregates |
| ZUSCULTAXCST012 | 1 transition | 2 LIFs moved | 6 aggregates |

ZUSCULTAXCST012 had minimal CIFS activity (no `Nblade.cifsWitnessFONotify` events) because no CIFS clients were connected during maintenance.
...
2. **Run `save-azure`** on test clusters:
   - ZUSCULTAXCST010 (has CIFS clients)
   - ZUSCULTAXCST011 (has CIFS clients)
   - ZUSCULTAXCST012 (no CIFS clients - verify empty fields)
3. **Query database** to verify new fields are populated:
   ...
   FROM azure_events
   WHERE cluster LIKE 'ZUSCULTAXCST01%';
   ```
