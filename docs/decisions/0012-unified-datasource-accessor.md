...
- **Build a thicker abstraction (full Repository pattern with query building, ORM, etc.).** Rejected: out of proportion for the project scope. `FieldMapping` already does the heavy lifting and the existing surfaces are thin enough that a thin convergence layer is sufficient.
