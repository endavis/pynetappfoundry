...
**Phase 2 implementation note (issue #541).** The Phase 2 generic `fetch()` dispatcher hardcodes the only known cross-model hook dependency: `compute_is_ha` on `CLUSTER_MAPPING` needs `results["nodes"]`. When a caller invokes `fetch(ClusterInfo, ...)` without supplying a `results_cache`, the dispatcher recursively fetches `OntapNodeResponse` to populate the dependency before running the hook. The check is by hook function name (`compute_is_ha`); no general declarative mechanism is introduced. A general `FieldMapping.depends_on=[<model_class>, ...]` mechanism is deferred until a second derived field with cross-model dependencies appears — at that point the second instance will give us enough variation to design a proper API. The collector's existing `_evaluate_derived_fields()` step is preserved unchanged so the ~80 mappings with intra-record derived fields keep their current evaluation path; only the `compute_is_ha` cross-model dependency is migrated to the inline fetch path in Phase 2.
